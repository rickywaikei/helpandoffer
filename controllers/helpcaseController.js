...
export const getAddHelpcase = (req, res) => {   //re-factoring 
  res.render("helpcases/add");
};
...
export const getRecords = (req, res) => {
  Helpcase.aggregate([
      ...
      if (res.locals.user.isAdmin) {
        console.log(getLogTime() + "admin account: " + res.locals.user.name + " read all cases");
...
export const putAddoffer = (req, res) => {
  ...
  offercase.offermsg = getTimestamp() + res.locals.user.name + ": " + req.body.offermsg; 
  offercase.save().then(() => {
...
export const putRemoveoffer = (req, res) => {
  Helpcase.findOne({ _id: req.params.id}).then((offercase) => {
  offercase.offer = null; 
  offercase.offermsg = getTimestamp() + res.locals.user.name + ": *** offerer role remove from this case ***" + "\n" + offercase.offermsg; 
  offercase.save().then(() => {
    req.flash("success_msg", "You are added as offerer the case !");
    res.redirect("/helpcases/offered");
  });
...
export const putGiveBadge = (req, res, next) => {
  ...
  User.findOne({ name: req.params.offer}).then((offeruser) => {
    if (offeruser.name != res.locals.user.name)
    offeruser.badge++;
    console.log(getLogTime() + res.locals.user.name + " gives a badge to " + offeruser.name);
    offeruser.save().then(() => {
